#version 430 core

#define MAX_DIST 25.0
#define ERROR 1.0
#define PRECIS 0.001
#define MAX_RAY_STEPS 32

in vec2 uv;
in vec3 uvw;
in vec4 ChunkCoord;
out vec4 color;

uniform sampler2D tex;
uniform vec3 camera;


float min3(vec3 x) {
	return min(min(x.x,x.y),x.z);
}
float max3(vec3 x) {
	return max(max(x.x,x.y),x.z);
}

float sdSphere(vec3 p, float r) {
	return length(p) - r;
}

float map(vec3 p) {
	return sdSphere(p - vec3(0.5 + ChunkCoord.xyz), 0.1);
}

float castRay( vec3 ro, vec3 rd, float tmin, float tmax) {
    float t = tmin;
    for( int i=0; i<MAX_RAY_STEPS; i++ ) {
	    float tempT = map( ro+rd*t);
        if( tempT<PRECIS || t>tmax ) break;
        t += tempT*ERROR;
    }

	return (t>tmax)? -1.0 : t;
}

vec3 calcNormal( in vec3 pos )
{
	vec2 eps = vec2( 0.001, 0.0 );

	vec3 nor = vec3( map(pos+eps.xyy).x - map(pos-eps.xyy).x,
					 map(pos+eps.yxy).x - map(pos-eps.yxy).x,
    				 map(pos+eps.yyx).x - map(pos-eps.yyx).x);
	return normalize(nor);
}

vec4 texcube(in vec3 p, in vec3 n )
{
	vec4 x = texture( tex, p.yz );
	vec4 y = texture( tex, p.zx ); 
	vec4 z = texture( tex, p.xy );
	return x*abs(n.x) + y*abs(n.y) + z*abs(n.z);
}

void main() {
	
	vec3 ro = camera;
	vec3 rd = normalize(uvw+ChunkCoord.xyz - camera);
	vec3 rs = sign(rd);
	vec3 ri = 1.0 / rd;


	vec3 entry = -sign(rd) * 0.5 + 0.5;
	vec3 exit = sign(rd) * 0.5 + 0.5;
	float tMin = max3( (entry + ChunkCoord.xyz - ro) * ri);
	float tMax = min3( (exit + ChunkCoord.xyz - ro) * ri);


	float t = castRay(ro, rd, tMin + 0.01, tMax);

	vec3 col = rd;
	vec3 hit = ro + rd*t;
	vec3 norm = calcNormal(hit);
	
	
	
	if(t > 0.0)
		color = vec4(1.0,0.0,0.0, 1.0);
	else
		discard;
	
	//color = texture(tex, TexCoords);
}